...
//                purchaseOrder = Invalid_Order();
        ...
        PurchaseOrder_SaveOrder(purchaseOrder);
        //Display order and order details
        DisplayOrder();
...
public void PurchaseOrder_SaveOrder(PurchaseOrderView purchaseOrder)
{
    ...
    if (errorlist.Count == 0)
    {
        if (activeOrder == null)
        {
            ...
            order.OrderDate = null; //the date needs to be null otherwise it will set automatially <---(its not working)
...
public PurchaseOrderView Add_New_Order()
{
    ...
    int item_One_ID = 102;
    int item_Two_ID = 103;
    int item_Three_ID = 104;

    purchaseOrder.VendorID = 2;
	...
	Console.WriteLine("Good Test Data (saving a new order)");
	purchaseOrder.Dump();
    return purchaseOrder;
}
